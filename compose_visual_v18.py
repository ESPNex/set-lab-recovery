...
R=Path(__file__).parent;O=Path('/home/user/Configurazioni-V18');O.mkdir(exist_ok=True)
...
for c in old['compositions']:
 ...
 owned=[i for i in items if i['set'] in pair and not i.get('excludedFromFinals')];selnames={i['file'] for i in selected.values()};inventory=[i for i in owned if i['file'] not in selnames];borrowed=[i for i in selected.values() if i['set'] not in pair]
 ...
 dr.rectangle((0,0,W,11),fill=BLUE);drawtxt(dr,(50,35),'SET LAB  /  POSIZIONAMENTO DA IMMAGINI  /  V18',21,BLUE);drawtxt(dr,(50,78),f"SET {pair[0]:02d} + {pair[1]:02d}",48);drawtxt(dr,(630,93),f"VARIANTE {c['variant']:02d}/{c['variantCount']:02d}",25)
 ...
 titles=['01  COVER','02  RETRO / TERMICA','03  TELAIO + NUCLEO','04  BORDI / FLEX','05  DISPLAY','06  VETRO']
 ...
 bat=selected['battery'];bs=bat.get('size') or [5.5,.4,6.8];local(bat,cx,top,0,3.25,min(bs[0],5.7),min(bs[2],6.4),'battery_visual',bat.get('angle') or 0);drawtxt(dr,(cx-150,top+15.8*S+12),label(bat),16)
 ...
 dr.line((48,1290,2245,1290),fill=BLUE,width=2);drawtxt(dr,(50,1310),'DETTAGLIO SEDI PCB',20,BLUE);boardlayer.thumbnail((620,335));can.alpha_composite(boardlayer,(50+(620-boardlayer.width)//2,1350));drawtxt(dr,(50,1697),label(board)+' • scala ingrandita; contatti non verificati',15)
 ...
 drawtxt(dr,(710,1310),'SEDI SOSPESE / MODULI DI DETTAGLIO',20,BLUE)
 ...
 drawtxt(dr,(50,1811),'NESSUN COLLAUDO FISICO • nessuna saldatura verificata • Fold e UTG dedicato esclusi • '+c['file'].replace('.png','-V18.png'),15)
 name=c['file'].replace('.png','-V18.png');can.convert('RGB').save(O/name,optimize=True)
 records.append({'file':name,'pair':pair,'variant':c['variant'],'selected':{role:path(i) for role,i in selected.items()},'selectionChanges':changes,'borrowed':[path(i) for i in borrowed],'placements':places,'coverRegionNormalized':coverroi[cover['sku']],'notes':notes,'electronicCompatibilityVerified':False,'physicalTestingPerformed':False,'visualReview':'PENDING','sha256':hashlib.sha256((O/name).read_bytes()).hexdigest()});print(name,flush=True)
coverage={path(i):sum(any(p['file']==path(i) for p in c['placements']) for c in records if i['set'] in c['pair']) for i in items if not i.get('excludedFromFinals')};assert all(coverage.values())
for c in records:
 for p in c['placements']:
  x,y,w,h=p['rectPx'];assert x>=0 and y>=0 and x+w<=W and y+h<=H,(c['file'],p)
manifest={'version':18,'basis':'Multimodal visual estimates; 5 PCB details and 4 shell/display/glass/frame atlases re-read. No engineering drawings inferred.','count':len(records),'canvas':[W,H],'nominalPixelsPerCm':S,'coverRegions':coverroi,'excludedSku':['disp-fold','glass-utg'],'activePairCoverage':coverage,'compositions':records};(O/'posizionamenti-v18.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2))
...
for first in range(1,17,2):
 ...
 sheet.save(R/f'analysis/v18-review-{first:02d}.jpg',quality=91)
...
(O/'GALLERIA.html').write_text('''<!doctype html><html lang="it"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SET Lab V18 — posizionamenti visivi</title><style>body{margin:0;background:#e9eff2;color:#142e3a;font:16px system-ui}header{padding:30px 4vw;background:#142e3a;color:white}header p{max-width:1000px;line-height:1.6}nav{padding:15px 4vw;position:sticky;top:0;background:#d8e3e9}select{font:inherit;padding:10px}main{padding:20px 3vw}article{margin-bottom:50px}img{width:100%;height:auto}h2{font-size:22px}</style><header><h1>29 proposte di posizionamento — V18</h1><p>Sei livelli: cover, retro/termica, telaio+nucleo, bordi/flex, display e vetro. Posizioni stimate dalle immagini, non misure di fabbricazione. Nessun collaudo dichiarato. Alternative e utensili restano separati. PNG originali 2800×1850 nella stessa cartella.</p></header><nav><select id="p"><option value="all">Tutte le coppie</option>'''+opts+'''</select></nav><main>'''+''.join(parts)+'''</main><script>document.querySelector('#p').onchange=e=>document.querySelectorAll('article').forEach(a=>a.hidden=e.target.value!=='all'&&a.dataset.pair!==e.target.value);</script></html>''')
